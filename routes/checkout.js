document.addEventListener('DOMContentLoaded', async function() {
  ...
  async function carregarDadosCheckout() {
    try {
      ...
      if (enderecoArea && dados.enderecos && dados.enderecos.length > 0) {
        ...
        enderecoArea.innerHTML = `<p class="texto-muted">Nenhum endereço cadastrado. <a href="perfil.html#enderecos">Adicione um endereço</a>.</p>`;
      }
  ...
  document.getElementById('btnAplicarCupomCheckout')?.addEventListener('click', async () => {
    ...
    try {
      const resp = await fetch(`${API}/api/cupons/validar`, {
